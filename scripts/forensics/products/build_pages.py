...
SECRET_RE = re.compile(r"((?:sk|rk)_(?:test|live)_|whsec_)([A-Za-z0-9]{10,})")
...
def build(slug: str, path: str) -> dict:
    ...
    # The reference code samples embed Stripe demo secret keys; never commit secret-shaped strings.
    raw = SECRET_RE.sub(lambda m: m.group(1) + "\u2022" * len(m.group(2)), raw)
